...
export const fetchAllProperties = createAsyncThunk(
  "sellar/fetchAllProperties",
  async ({ id, isapproved, page, limit }, { rejectWithValue }) => {
    try {
      const response = await GetAllproperties({id, isapproved, page, limit});
...
export const fetchAllSellarscount = createAsyncThunk(
  ...
  async (status, { rejectWithValue }) => {
    try {
      const response = await GetAllpropertiescount(status);
      if (response?.status === false) {
        return rejectWithValue(response.message);
      }
      return response.data;
