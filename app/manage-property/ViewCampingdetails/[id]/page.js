...
export default function CampingDetailsPage() {
  ...
  const handleVillaStatus = async (status) => {
    ...
    try {
      let result = await fetch(`${BaseUrl}/Camping/approve-reject/${villaId}`, {
        method: "PUT",
        headers: {
    ...
    return (
      <div className="w-full flex justify-center items-center h-screen">
        <div className="text-center">
          ...
          <Button className={"bg-teal-700 text-white hover:bg-teal-700"} onClick={() => dispatch(fetchvillabyid(id))}>Retry</Button>
        </div>
      </div>
    );
  }
